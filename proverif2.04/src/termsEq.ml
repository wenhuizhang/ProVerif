...
let rec complete_eq bothdir =
  ...
  | Some eq ->
	...
	begin
          Display.Text.display_eq_indep eq;
...
let buildblocks eqlists =
  ...
  List.iter (fun (f1,l1,_) ->
	...
	begin
	  ...
	  Display.Text.display_item_list Display.Text.display_eq_indep l1;
	  print_string "and";
	  Display.Text.display_item_list Display.Text.display_eq_indep l2;
  ...
  begin
	...
	EqNoInfo ->
	  ...
	  with
		...
		begin
		  ...
		  Display.Text.display_item_list Display.Text.display_eq_indep block;
		  ...
		  begin
			...
			Display.Text.display_eq_indep eq1;
			print_string "\nand\n";
			Display.Text.display_eq_indep eq2;
			print_string ".)\n"
    ...
    if !Param.html_output then
      begin
        ...
        if !linear_part = []
        then Display.Html.print_line "No equation."
        else
          begin
            Display.Html.print_string "Initial equations:";
            Display.Html.display_item_list Display.Html.display_eq_indep (!linear_part)
          end
      end
    else if !Param.verbose_eq then
      begin
        if !linear_part = []
        then Display.Text.print_line "Linear part: No equation."
        else
          begin
            Display.Text.print_string "Linear part:";
            Display.Text.display_item_list Display.Text.display_eq_indep (!linear_part)
          end
      end;
    List.iter (fun eq ->
       add_eq eq;
    ...
    eq_base := [];
    if !Param.html_output && !linear_part != [] then
      begin
        Display.Html.print_string "Completed equations:";
        Display.Html.display_item_list Display.Html.display_eq_indep resulting_equations_linear
      end
    else if !Param.verbose_eq && !linear_part != [] then
      begin
        Display.Text.print_string "Completed equations:";
        Display.Text.display_item_list Display.Text.display_eq_indep resulting_equations_linear
      end;
    ...
    if !Param.html_output then
      begin
        ...
        then Display.Html.print_line "No equation."
          ...
          begin
            Display.Html.print_string "Initial equations:";
            Display.Html.display_item_list Display.Html.display_eq_indep (!convergent_part)
          end
      end
    else if !Param.verbose_eq then
      begin
        if !convergent_part = []
        then Display.Text.print_line "Convergent part: No equation."
        else
          begin
            Display.Text.print_string "Convergent part:";
            Display.Text.display_item_list Display.Text.display_eq_indep (!convergent_part)
          end
      end;
    ...
    if !Param.html_output && !convergent_part != [] then
      begin
        Display.Html.print_string "Completed equations:";
        Display.Html.display_item_list Display.Html.display_eq_indep resulting_equations_convergent
      end
    else if !Param.verbose_eq && !convergent_part != [] then
      begin
        Display.Text.print_string "Completed equations:";
            Display.Text.display_item_list Display.Text.display_eq_indep resulting_equations_convergent
      end;
  ...
  begin
    ...
    if !Param.verbose_eq then
      begin
        ...
        Display.Text.display_item_list Display.Text.display_eq_indep (!rewrite_system)
      end;
	...
	Display.Text.display_item_list Display.Text.display_eq_indep resulting_equations
      end;
    List.iter (function
      (FunApp(f, l), req) as eq ->
        begin
...
let syntactic_table = HashtblSymbol.create 7

let get_syntactic f =
  try
    HashtblSymbol.find syntactic_table f
  with Not_found ->
    let f' = { f_name = (* "sy_" ^ *)
                 begin
                   match f.f_name with
               ...
               f_record = Param.fresh_record () } in
...
let copy_remove_syntactic_e (t,ext) = (copy_remove_syntactic t, ext)

let copy_remove_syntactic_option = function
  | None -> None
  | Some t -> Some (copy_remove_syntactic t)

let copy_remove_syntactic_e_option = function
  | None -> None
  | Some t -> Some (copy_remove_syntactic_e t)

let copy_remove_syntactic_event = function
  | QSEvent(inj,ord_fun,occ,at,t) -> QSEvent(inj,ord_fun,copy_remove_syntactic_option occ,copy_remove_syntactic_e_option at,copy_remove_syntactic t)
  | QSEvent2(t1,t2) -> QSEvent2(copy_remove_syntactic t1, copy_remove_syntactic t2)
  | QFact(p,ord_fun,args,at) -> QFact(p,ord_fun,List.map copy_remove_syntactic args,copy_remove_syntactic_e_option at)
  | QNeq(t1,t2) -> QNeq(copy_remove_syntactic_e t1, copy_remove_syntactic_e t2)
  | QEq(t1,t2) -> QEq(copy_remove_syntactic_e t1, copy_remove_syntactic_e t2)
  | QGeq(t1,t2) -> QGeq(copy_remove_syntactic_e t1, copy_remove_syntactic_e t2)
  | QGr(t1,t2) -> QGr(copy_remove_syntactic_e t1, copy_remove_syntactic_e t2)
...
(* [occur_check_eq_modulo_aux restwork restwork_occurs v t] calls [restwork t'] where
   [t'] is a term equal to [t] and that does not contain [v]. When [restwork t'] raises
   [Unify], tries calling [restwork] another such value of [t']. When no [t'] equal to [t],
   not containing [v], and such that [restwork t'] does not raise Unify can be found,
   and [t] contains [v], calls [restwork_occurs()]. *)

(* The following functions take as argument three functions to handle
 exceptions Unify and NoBacktrack in a tail recursive way :
  - f_clean : the function that cleans the link of variables when Unify and NoBacktrack
      have not been raised. It must be call before returning the result
      (see functions close_term_eq_synt and unify_modulo_list)
  - f_Unify : the function to call when "raising" Unify
  - f_NoBacktrack : the function to call when "raising" NoBacktrack *)

let auto_cleanup_eq_tail f_next f_clean f_Unify f_NoBacktrack =
  ...
  | Var v' ->
      begin match v'.link with
        ...
        | _  ->
            ...
            else restwork f_clean f_Unify f_NoBacktrack t
      end
  | FunApp(f,args) ->
      if f_has_no_eq f
      then
                ...
                | (leq, req) :: lrew ->
                    ...
                    unify_modulo_list_tail (fun f_clean_2 f_Unify_2 f_NoBacktrack_2 ->
                      occur_check_eq_modulo_aux_tail restwork (fun f_clean_3 f_Unify_3 f_NoBacktrack_3 ->
                        ...
                        f_Unify_3 ()
                      ) f_clean_2 f_Unify_2 f_NoBacktrack_2 v req'
                    ) f_clean_1
                    (fun () ->
                      (* In that case the unification modulo did not work *)
                      reweqlist lrew
                    ) f_NoBacktrack_1 args leq'
                | [] -> restwork_occurs f_clean_1 f_Unify_1 f_NoBacktrack_1
              in
              reweqlist eqlist
...
and occur_check_list_eq_modulo_tail restwork restwork_occurs f_clean f_Unify f_NoBacktrack v l =
  ...
  let rec occur_check_list_eq_modulo_rec restwork f_clean f_Unify f_NoBacktrack = function
    ...
    | t::q ->
        ...
        ) (fun f_clean_1 f_Unify_1 f_NoBacktrack_1 -> occurs_in_l := true; f_Unify_1 ())
        f_clean f_Unify f_NoBacktrack v t
  in

  occur_check_list_eq_modulo_rec restwork f_clean (fun () ->
    if !occurs_in_l
    then restwork_occurs f_clean f_Unify f_NoBacktrack
    else f_Unify ()
  ) f_NoBacktrack l
  ...
  let rec add_unif_term restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right t1 t2 =
    match t1, t2 with
      FunApp(f1, l1), FunApp(f2,l2) when f_has_no_eq f1 && f_has_no_eq f2 ->
        if (non_syntactic f1) != (non_syntactic f2)
        then f_Unify ()
        else add_unif_term_list restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right l1 l2
    ...
    | (Var v, _) ->
          begin
            ...
            | TLink t1' -> add_unif_term restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right t1' t2
          ...
          begin
            ...
            | NoLink ->
                ...
                then f_Unify ()
                  ...
                  ) f_clean f_Unify f_NoBacktrack v t1
      ...
      | _ -> restwork f_clean f_Unify f_NoBacktrack (t1::unif_to_do_left) (t2::unif_to_do_right)

  and add_unif_term_list restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right l1 l2 = match l1,l2 with
    | [], [] -> restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right
    | [], _ | _, [] -> Parsing_helper.internal_error "List should be of same size"
    | t1::q1, t2::q2 ->
        add_unif_term (fun f_clean_1 f_Unify_1 f_NoBacktrack_1 unif_to_do_left_1 unif_to_do_right_1 ->
          add_unif_term_list restwork f_clean_1 f_Unify_1 f_NoBacktrack_1 unif_to_do_left_1 unif_to_do_right_1 q1 q2
        ) f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right t1 t2
  in
...
(* The function close_term_eq_synt and unify_modulo_list respectively call
  their tail recursive functions close_term_eq_synt_tail and unify_modulo_list_tail.
  As the latter functions have been implemented without any try with, it is
  important to catch the possible exception raised during restwork.
  When the exception are Unify or NoBacktrack, it suffices to call the respective
  function f_Unify and f_NoBacktrack. However, for any other exception, we need
...
let close_term_eq_synt restwork t =
  close_term_eq_synt_tail (fun f_clean f_Unify f_NoBacktrack t' ->
    ...
    with
      | Unify -> f_Unify ()
      | NoBacktrack r -> f_NoBacktrack r
      | x ->
          f_clean ();
          raise x
  ) (fun () -> ()) (fun () -> raise Unify) (fun r -> raise (NoBacktrack r)) t

let unify_modulo_list (restwork:unit -> 'a) (l1:term list) (l2:term list) =
...
let algo_BellmanFord restwork rel_list =
  ...
  let edges =
    List.map (fun (t1,n,t2) ->
      let k1 = add_vertex t1 in
      let k2 = add_vertex t2 in
      (k2,k1,n)
    ) rel_list
  in
...
let remove_nat_diseq_neq_conj_exc f_next f_next_geq f_next_inst nat_vars neq =
  ...
  let rec explore_disj f_next f_next_geq f_next_inst = function
    | [] -> f_next false
    | (Var v1,t2)::q ->
        (* Since the disequations are simplified, we only have two cases:
	   - t1 is a variable, handled here
	   - t1 is replaced with any_val, handled below
    ...
    | _::q -> explore_disj f_next f_next_geq f_next_inst q
  in
...
(** Remove disequalities of the form x <> caught-fail **)

let remove_caught_fail constra =

  let is_caught_diseq (t1,t2) = match t1,t2 with
    | Var x, t | t, Var x ->
        equal_terms t (FunApp(glet_constant_fun x.btype,[]))
    | _ -> false
  in
  ...
  let rec remove = function
    | [] -> []
    | diseq::q ->
        if List.exists is_caught_diseq diseq
        then remove q
        else diseq :: (remove q)
  in
  { constra with neq = remove constra.neq}
...
let simple_simplify_constraints c =
  let c1 = Terms.copy_constra4 c in
  let vars = ref [] in
  Terms.iter_constraints (Terms.get_vars vars) c1;
  simplify_constraints_keepvars
    ...
    (fun c2 ->
      ...
      List.fold_left (fun acc v -> match v.link with
        ...
        | _ -> Parsing_helper.internal_error "[termsEq.ml >> simplify_constraints4] variable should be linked."
      ) c2 !Terms.current_bound_vars
    )
    !vars c1
...
let simple_simplify_constraints_rule (hyp,concl,hist,c) =
  let vars = get_vars_facts (concl::hyp) in
  simplify_constraints_keepvars
    (fun c2 -> (hyp,concl,hist,c2))
    (fun c2 ->
      (* The variables in !Terms.current_bound_vars are the variables
         that are made equal by the inequalities. We added inequalities to
         represents these equalities. *)
...
let simplify_constraints_rule f_next f_next_inst ((hypl,concl,hist,constra) as rule)=
  if Terms.is_true_constraints constra
  then f_next rule
  else
    try
      if constra.geq = []
...
let hasConvergentEquations () = !rewrite_system != []

let rec simp_eq = function
    | Var v -> ()
    | (FunApp(f,l)) as t ->
        List.iter simp_eq l;
        ...
        let rec find_red = function
          ...
          | ((leq,req)::redl) ->
              try
                if not (Terms.equal_types (Terms.get_term_type leq) (Terms.get_term_type t))
                then raise NoMatch;
...
(***** Check if terms have distinct subterms but equal modulo equational theory. *****)
...
let exists_distinct_subterms_equal_modulo term_list =
  Terms.auto_cleanup (fun () ->
    ...
    let equal_modulo t t' =
      try unify_modulo (fun () -> true) t t' with Unify -> false
    in

    (* The list [subterm_list] contains the list of subterms that we have already
       checked. In particular, if a term [t] is equal syntactically to a term in
       [subterm_list] then all subterms of [t] occur in [subterm_list]. *)
    let rec find_subterm f_next subterm_list = function
      | [] -> f_next subterm_list
      | t::q ->
          if List.exists (Terms.equal_terms t) subterm_list
          then find_subterm f_next subterm_list q
          else
	    match t with
	    | FunApp(_,args) ->
		find_subterm (fun subterm_list' ->
              (* [subterm_list'] now contains all distinct subterms of [args].
                We now check that [t] is not equal modulo the equational theory
                to [subterm_list']. Note that [subterm_list'] is the union of
                [subterm_list] and [args] (in the set theoretic sense) so we are
                guaranteed that [t] is distinct from all terms in [subterm_list'].
              *)
		  if List.exists (equal_modulo t) subterm_list'
		  then true
		  else find_subterm f_next (t::subterm_list') q
		      ) subterm_list args
	    | Var _ ->
		if List.exists (equal_modulo t) subterm_list
		then true
    ...
    find_subterm (fun _ -> false) [] term_list
  )
          ...
          close_constra_eq (fun side_c0 ->
            ...
            begin
              try
                ...
                (* We check if the rewrite rule is correctly normalised *)
                List.iter simp_eq leq2;
                simp_eq req2;

                (* Eliminate redundant rewrite rules *)
                if exists_distinct_subterms_equal_modulo (req2::leq2) ||
                  List.exists (fun rew3 ->
                    implies_rew rew3 rew2
                  ) (!results) then ()
                else
                  results := rew2 :: (List.filter (fun rew3 ->
