    ...
    auto_cleanup (fun () ->
        ...
        try
		  ...
		  let state' = Menu_helper.add_public_list_dec_all_mode state recipe_term_list in
                  if state' == state then
                    let _ = Menu_helper.question_box  ~title:"Already in public" ~buttons:["Ok"] ("Term " ^ (Display_interact.GtkInteract.display_term result) ^ " (or its components after decomposing data constructors) already in public") in
                    state
                  else
        ...
        | Unify ->
           let _ = Menu_helper.question_box ~title:"Evaluation fails on both sides" ~buttons:["Ok"]
    ...
    begin
      ...
      | Unify ->
         if not (Terms.equal_types (Terms.get_pat_type pat) (Terms.get_term_type result)) then
           ignore (Menu_helper.question_box ~title:"Input failed" ~buttons:["OK"] "The input failed because the received message is not of the expected type." )
         else
           ignore (Menu_helper.question_box ~title:"Input failed" ~buttons:["OK"] "The input failed because the pattern-matching failed.");
         { state with
