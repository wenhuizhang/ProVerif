...
let rec transl_pat next_f cur_state under_choice pattern =
  ...
  | PatTuple(fsymb,pat_list) when fsymb.f_cat = Choice ->
      ...
      if cur_state.cur_phase < !min_choice_phase then
	Parsing_helper.user_error ("diff or choice pattern in phase "^(string_of_int cur_state.cur_phase)^" but diff/choice terms occur only in phase at least "^(string_of_int (!min_choice_phase)));
  ...
  verify_explored_occurrence cur_state process (fun () ->
    ...
    | Input(tc,pat,proc,occ) ->
		...
		let build_hyp term_pat_left term_pat_right cur_state =
		  ...
		  | Some(ev, occ_name) ->
                      (fact1::Pred(Param.begin2_pred,[FunApp(ev,[occ_name;term_pat_left]);FunApp(ev,[occ_name;term_pat_right])])::cur_state.hypothesis,
