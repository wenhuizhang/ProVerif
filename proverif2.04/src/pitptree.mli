...
type extended_equation =
  | EELet of ident * term_e * extended_equation
  | EETerm of term_e
...
type nounif_t =
  ...
  | BFNoUnif of (ident * gformat_e list * int)

(* Queries *)

type gterm =
    PGIdent of ident
  | PGFunApp of ident * gterm_e list * ident option
  | PGPhase of ident * gterm_e list * int * ident option
...
and tpattern =
  ...
  | PPatChoice of ident * tpattern list * ident option(*type*)
...
type tprocess =
  ...
  | PLetDef of ident * pterm_e list * ident option
...
type tdecl =
  ...
  | TReduc of (envdecl * extended_equation) list * options list(*options*)
  | TReducFail of ident * ident list * ident * (may_fail_env_decl * extended_equation) list * options list(*options*)
  | TEquation of (envdecl * extended_equation) list * options list(*options*)
  ...
  | TQuery of envdecl * tquery_e list * options list (* options *)
  | TNoninterf of envdecl * (ident * term_e list option) list
  | TWeaksecret of ident
  | TNoUnif of envdecl * nounif_t * Types.nounif_value * options list(*options*)
