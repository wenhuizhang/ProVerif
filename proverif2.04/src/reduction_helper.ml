...
let prove_att_phase_tquery n = function
  | CorrespQuery(ql,_) ->
      List.exists (fun (q,_) -> prove_att_phase_query n q) ql
  | CorrespQEnc(qql,_) ->
      List.exists (fun (_,(q,_)) -> prove_att_phase_query n q) qql
  | QueryToTranslate _ ->
...
||
  (List.exists (function
    ...
    | Lemma lem ->
	match lem.is_axiom with
	| KRestriction ->
	    List.exists (fun lem1 ->
	      match fst lem1.ql_query with
	      | PutBegin _ -> false
	      | QSecret _ -> Parsing_helper.internal_error "Pitransl.prove_att_phase: lemma cannot be QSecret"
	      | RealQuery(Before(hyp,_),_) ->
		  (* If the restriction contains attacker(...) phase n' in its premise,
		     we also apply is for all phases n <= n'. For all these phases,
		     we must not optimize mess(...) into attacker(...). *)
		  List.exists (is_att_phase_greater n) hyp) lem.lemmas
	| KAxiom | KLemma ->
	    (* For lemmas and axioms, we can apply them on an IO-compliant trace,
               which sends messages on public channels through the adversary, so
               we can optimize mess(...) into attacker(...) even if
               attacker(...) appears in the premise of a lemma or axiom. *)
	    false
    ) pi_state.pi_lemma)
...
and conclusion. Stops with an error if this is not possible.
  This function is applied before the correspondence query is encoded
thus it should not contain occurrence variables nor ordering functions.
*)

let rec has_inj_conclusion_query = function
  | QEvent(QSEvent(Some _,_,_,_,_)) -> true
  | QEvent _
  | QTrue
  | QFalse -> false
...
and has_inj = function
  | Before(evl,concl_q) ->
      ...
      List.exists (function QSEvent(Some _,_,_,_,_) -> true | _ -> false) evl

let rec split_events_list inj_events events othergoals = function
  | [] -> List.rev inj_events, List.rev events, List.rev othergoals
  | (QSEvent(Some _,_,_,_,_) as inj_goal)::rest -> split_events_list (inj_goal::inj_events) events othergoals rest
...
let remove_injectivity = function
  | QSEvent(_,ord_fun,occ,at,ev) ->
      assert(ord_fun = []);
      assert(occ = None);
      QSEvent(None,[],None,at,ev)
  | ev -> ev
      ...
      then
        let evl' =
          match split_events_list [] [] [] evl with
            ...
            | [], [QSEvent(None,ord_fun,occ,at,ev)], othergoals ->
                assert(ord_fun = []);
                assert(occ = None);
                (QSEvent(Some (Param.fresh_injective_index ()), [],None,at,ev)) :: othergoals
...
let rec occurs_var_pat v = function
  | PatVar _ -> false
  | PatTuple(_,args) -> List.exists (occurs_var_pat v) args
...
let include_arg (env, args, need_list) m always =
  ...
  | MVar(b,_) ->
     ...
     begin
       match args with
         None ->
          if (always = Always && (Param.get_names_with_args())) || found_in_need_list then Some(new_meaning) else None
       | Some args_content ->
...
(* [get_until_pos pos l] returns the first [pos] elements of [l]. *)
let rec get_until_pos pos l =
  if pos = 0
  then []
  else
    match l with
      | [] -> Parsing_helper.internal_error "[reduction_helper.ml >> get_until_pos] Position outside of bound of the list"
      | t::q -> t::(get_until_pos (pos-1) q)
...
| LetFilterTagElse o -> print_string "se"; print_string (string_of_int o.occ)
...
let match_equiv next_f f1 f2 =
  ...
  | Pred(p1,[t1]), Pred(p2, [t2';t2])
	...
	&& Terms.is_ground_public t2' -> match_modulo next_f t2 t1
  | Pred(p1,[t1;t2]), Pred(p2, [t3';t3;t4';t4])
        when (corresp_att_mess_bin p1 p2)
	&& Terms.is_ground_public t3'
        && Terms.equal_terms t3' t4' -> match_modulo_list next_f [t3;t4] [t1;t2]
  | _ -> raise Unify
...
(* After the translation of the query in pisyntax/pitsyntax,
   ...
   These functions are applied after the queries have been translated. Thus
   correspondence queries can contain ordering functions and occurrence variables.
   Moreover, even though they are applied after encoding, they are also applied
...
let check_delayed_names_e = function
    QSEvent(b,ord_fun,occ,at,t) -> QSEvent(b,ord_fun,check_delayed_names_t_op occ,check_delayed_names_te_op at,check_delayed_names_t t)
  | QSEvent2(t1,t2) -> QSEvent2(check_delayed_names_t t1, check_delayed_names_t t2)
  | QFact(p,ord_fun,tl,at) -> QFact(p,ord_fun, List.map check_delayed_names_t tl,check_delayed_names_te_op at)
...
let create_pdf_trace a_to_term noninterf_test_to_string inj_string final_state =
  if !Param.html_output && !Param.graph_output then
    ...
    if !Param.trace_backtracking &&
      ((!Param.html_output) || (!Param.trace_display_graphicx = Param.GraphDisplay)) &&
      (!Param.trace_display <> Param.NoDisplay) then
      begin
...
let is_success_corresp_goal = function
    CorrespGoal (l,_) -> List.for_all is_success_corresp_goal1 l
  | _ -> false
...
let has_name_e = function
    QSEvent(_,_,_,_,t) -> has_name_t t
  | QSEvent2(t,t') -> has_name_t t || has_name_t t'
  | QGr _ -> Parsing_helper.internal_error "[reduction.ml >> has_name_e] QGr should not occur in query during reduction."
  | QIsNat t -> has_name_t t
  | QFact(_,_,l,_) -> List.exists has_name_t l
  | QNeq((t1,_),(t2,_))
  | QEq((t1,_),(t2,_))
  | QGeq((t1,_),(t2,_)) -> (has_name_t t1) || (has_name_t t2)
