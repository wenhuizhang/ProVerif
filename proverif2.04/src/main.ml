...
let has_query pi_state =
  match pi_state.pi_process_query with
  | Equivalence _ -> true
  | SingleProcess(_,ql) -> ql != []
  | SingleProcessSingleQuery _ -> true
...
let check_delayed_names_query = function
  | QueryToTranslate _ ->
...
let check_delayed_names pi_state =
  let process_query' =
    match pi_state.pi_process_query with
    | (Equivalence _) as pq -> pq
    | SingleProcess(p,ql) -> SingleProcess(p, List.map check_delayed_names_query ql)
    | SingleProcessSingleQuery(p,q) -> SingleProcessSingleQuery(p, check_delayed_names_query q)
  in
  let t_lemmas_list' =
    List.map (function
  ...
  in
  let original_axioms' = List.map (fun (ax,b) -> Reduction_helper.check_delayed_names_ax ax, b) pi_state.pi_original_axioms in
  { pi_state with
    pi_process_query = process_query';
...
let interference_analysis horn_state pi_state =
  ...
  if result = [] then
    ...
    begin
      let traces_to_recontruct = ref (!Param.reconstruct_trace) in
      let shown_stop = ref false in
	  ...
	  let new_tree =
		...
		begin
		  ...
		  Display.Html.display_rule_abbrev rule
              ...
              let deriv = History.build_history None rule in
	      if (!traces_to_recontruct != 0) && (!Param.reconstruct_derivation) then
		History.unify_derivation None deriv
	      else
		deriv
		)
	  in
          ...
          (* For weak secrets, the reconstructed attack really falsifies the
            ...
            if (!traces_to_recontruct != 0) && (!Param.reconstruct_derivation) then
	      begin
		if !traces_to_recontruct > 0 then decr traces_to_recontruct;
		let res =
		  match query with
		  | WeakSecret _ ->
                      Reduction.do_reduction None pi_state.pi_original_axioms new_tree
		  | NonInterfQuery _ ->
                      ignore (Reduction.do_reduction None pi_state.pi_original_axioms new_tree);
                      false
		  | ChoiceQuery | ChoiceQEnc _ ->
                      ignore (Reduction_bipro.do_reduction None pi_state.pi_original_axioms new_tree);
                      false
		  | _ ->
                      false
		in
		if (not res) && (!traces_to_recontruct = 0) && (!Param.reconstruct_trace > 0) && (not !shown_stop) then
		  begin
		    shown_stop := true;
...
let display_axioms pi_state =
  if !current_axioms = [] then
    let process = Reduction_helper.get_process pi_state in
    List.iter (function
      | Lemma ({ is_axiom = KAxiom | KRestriction } as axiom) ->
          let axiom_query = check_delayed_names_query (convert_lemma_to_query axiom) in
          let axiom_process = SingleProcessSingleQuery(process,axiom_query) in
          let pi_state1 =
            { pi_state with
              pi_process_query = axiom_process }
          in
          let is_axiom = axiom.is_axiom = KAxiom in
          let q_type = if is_axiom then Display.TAxiom else Display.TRestriction in
          current_axioms := (axiom_process,is_axiom) :: (!current_axioms);
          display_query_header q_type pi_state1
      | Lemma _ -> ()
      | _ -> Parsing_helper.internal_error "[main.ml >> prove_lemmas] All lemmas should have been translated at this point. (2)"
	    ) pi_state.pi_lemma

let solve pi_state =
  (* We save the tuples that will be used in the translation. It avoids
     generating clauses for useless tuples when there are multiple queries. *)
  let tmp_tuple = Terms.extract_tuple_table() in
  ...
  (* We display axioms in the initial state (before simplification
     of lemmas), and only for the first execution with this set of lemmas/axioms,
     but we do that after check_delayed_names, so that names are correctly displayed *)
  display_axioms pi_state;
  ...
  Terms.restore_tuple_table tmp_tuple;
  result
...
let display_summary results =
  ...
  then
    begin
      Display.Text.print_line "No query in the input file.";
      if !Param.html_output
      then Display.Html.print_string "\n<LI>No query in the input file."
    end;
  ...
  List.iter (fun sum ->
	...
	let grouped_axioms = ref [] in
        let grouped_restrictions = ref [] in
	List.iter (fun (pq,b) -> match pq with
	  | SingleProcessSingleQuery(p,axiom) ->
		...
		with Not_found ->
		  ...
		  in
		  if b
                  then grouped_axioms := new_g :: (!grouped_axioms)
                  else grouped_restrictions := new_g :: (!grouped_restrictions)
	      end
	  | _ -> assert false
		) sum.sum_axioms;
	...
	Display.Text.display_summary queries' (!grouped_axioms) (!grouped_restrictions) (!grouped_lemmas);
	Display.Text.newline();
	if !Param.html_output
	  then
	  begin
	    Display.Html.print_string "\n<LI>";
	    Display.Html.display_summary queries' (!grouped_axioms) (!grouped_restrictions) (!grouped_lemmas);
	  end
...
let rec is_axiom_filtering_needed proc =
  ...
  then
    match proc.construction with
      ...
      | Simplify _
      | BarrierSwap _ -> true
      | Move(_, proc') -> is_axiom_filtering_needed proc'
      | Initial_process1
...
let state_has_restrictions pi_state =
  List.exists (function
    | Lemma { is_axiom = KRestriction } -> true
    | Lemma { is_axiom = KAxiom | KLemma } -> false
    | _-> Parsing_helper.internal_error "[main.ml >> state_has_restrictions] All lemmas should have been translated at this point."
	  ) pi_state.pi_lemma
...
let prove_lemmas pi_state next_f =
  ...
  let axiom_list,lemma_list =
    List.partition (function
      | Lemma { is_axiom = KAxiom | KRestriction; _} -> true
      | Lemma { is_axiom = KLemma; _} -> false
...
let prove_equivalence_with_choice pi_state =
  ...
  let pi_state1 = Move_new_let.move false pi_state1 in

  group_summary := { sum_queries = [pi_state.pi_process_query, DontKnow];
		     sum_lemmas = [];
		     sum_axioms = [] };
  let has_restrictions = state_has_restrictions pi_state1 in
  Param.has_restrictions := has_restrictions;
  begin
  try
	...
	open_list_opt (pi_state4.pi_lemma <> []) pi_state4 (fun pi_state4 ->
          let apply_simplification () =
            if !Param.typed_frontend && not (!Param.has_barrier) &&
	      not (!Param.allow_diff_patterns)  && not has_restrictions then
              begin
	         ...
	         Do not simplify processes when we have diff patterns: simplification does
		 not preserve diff-equivalence, which we want to prove in this case. *)
...
let prove_equivalence_two_processes pi_state =
  ...
  if state_has_restrictions pi_state then
    Parsing_helper.user_error "Restrictions are not allowed when ProVerif proves the equivalence of two processes";
  ...
  try
    ...
    Simplify.simplify_state pi_state1 (fun pi_state2 ->
      (* After simplification, we know that lets never fail.
	 We pass this information to [Move_new_let.move] to strengthen it. *)
      let pi_state2 = Move_new_let.move true pi_state2 in
...
let anal_file s0 =
  ...
  let in_front_end = match !in_kind with
    | Default -> (* Set the front-end depending on the extension of the file *)
       ...
       if StringPlus.ends_with s_up ".PV" then PiType else
       if StringPlus.ends_with s_up ".PI" then Pi else
       if StringPlus.ends_with s_up ".HORNTYPE" then HornType else
  ...
  begin
    ...
    | PiType ->
        Param.typed_frontend := true;
        let pi_state1 = Pitsyntax.parse_file s in
        let pi_state2 = Simplify.prepare_process pi_state1 in
    ...
    | Pi ->
        let pi_state1 = Pisyntax.parse_file s in
...
let dummy_param = ("", dummy_ext)
let dummy_val = S("",dummy_ext)

let add_param s =
  let s' = (s,dummy_ext) in
  match !Param.additional_settings with
  | [] -> Param.additional_settings := (s', dummy_val) ::(!Param.additional_settings)
  | (p,v)::r ->
      assert (v != dummy_val);
      if p == dummy_param then
	Param.additional_settings := (s', v) :: r
      else
	Param.additional_settings := (s', dummy_val) ::(!Param.additional_settings)
...
let add_value s =
  ...
  | (p,v)::r ->
      assert (p != dummy_param);
      if v == dummy_val then
	Param.additional_settings := (p,v') :: r
      else
	Param.additional_settings := (dummy_param, v') :: (!Param.additional_settings)
...
let _ =
  try
    ...
    [ "-test", Arg.Unit (fun () ->
      ...
      "-set", Arg.Tuple [Arg.String add_param; Arg.String add_value],
      "<param> <value> \tequivalent to adding set <param> = <value> to the input file";
