...
let error_box mess =
  let _ =  question_box ~title:"Error" ~buttons:["OK"] mess in
  ()

let input_error_box b mess ext =
  error_box (Parsing_helper.get_mess_from b "Error: " mess ext)
...
let parse_term s =
  ...
  if warnings != [] then
    begin
      ...
      match (question_box ~title:"Warnings" ~buttons:["Yes"; "No"] messages ) with
	0 | 1 -> ()
