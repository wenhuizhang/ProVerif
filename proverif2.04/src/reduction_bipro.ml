...
let find_io_rule next_f hypspeclist hyplist name_params var_list io_rules =
  ...
  let rec find_io_rule_aux = function
    [] -> raise Unify
  | (((n, sons, hypspeclist2, name_params',_) as io_rule)::io_rules) ->
  ...
  | FunApp(f,[c;tthen;telse]) when
    f == Terms.gtest_fun (snd f.f_type) ->
	 ...
	 useful in particular to avoid evaluating catch-fail(caught-fail). *)
      let c_eval = term_evaluation side c in
      begin
        try
          auto_cleanup (fun () ->
            Reduction_helper.match_modulo (fun () ->
              term_evaluation side tthen
        ...
        with Unify ->
          ...
          with Unify -> term_evaluation side telse
      end
  | FunApp(f,l) ->
...
let make_name_param_entry cur_state t' =
  let t =
...
let term_evaluation_name_params cur_state occ t name_params =
  ...
  if may_have_several_patterns then
    t', ((make_name_param_entry cur_state t') :: name_params)
  else
    t', name_params
...
let term_evaluation_name_params_true cur_state occ t name_params =
  let may_have_several_patterns = reduction_check_several_patterns occ in
  let t' = bi_action (term_evaluation_to_true t) in
  if may_have_several_patterns then
    (make_name_param_entry cur_state t') :: name_params
  else
    name_params

(* Update the value of name_params after pattern-matching *)

let rec update_name_params cur_state put_var name_params = function
    PatVar b ->
      let bi_t = copy_closed_remove_syntactic (Var b) in
      let t =
	if cur_state.current_phase < get_min_choice_phase() then
	  match bi_t with
	  | FunApp({ f_cat = Choice }, [t1;t2]) -> t1
	  | _ -> Parsing_helper.internal_error "reduction_bipro: variable should be linked to choice"
	else
	  bi_t
      in
      (MVar(b, None), t, put_var)::name_params
  | PatTuple(f,l) -> update_name_params_list cur_state put_var name_params l
  | PatEqual _ -> name_params

and update_name_params_list cur_state put_var name_params = function
    [] -> name_params
  | (a::l) -> update_name_params_list cur_state put_var (update_name_params cur_state put_var name_params a) l
...
let rec match_pattern p side t =
  ...
  | PatTuple(f,[pat1;pat2]) when f.f_cat = Choice ->
      if side = 1 then
	begin
	  match_pattern pat1 1 t;
	  match_pattern pat1 2 t;
	end
      else
	begin
	  match_pattern pat2 1 t;
	  match_pattern pat2 2 t;
	end
  | PatTuple(f,l) ->
...
let term_evaluation_name_params_and_match cur_state pat occ t name_params =
  let may_have_several_patterns = reduction_check_several_patterns occ in
...
let rec decompose_tuple = function
  | (FunApp({f_cat = Tuple } as f1,l1), FunApp(f2,l2)) when f1 = f2 ->
      List.concat (List.map2 (fun t1 t2 -> decompose_tuple (t1, t2)) l1 l2)
  | x -> [x]


let rec can_be_built public t =
  try
    let (ca, _) = List.find (fun (_, t') -> equal_bi_terms_modulo t t') public in
    Some ca
  with Not_found ->
    match t with
    | FunApp({f_private = false; _} as f1,l1), FunApp(f2, l2) when f1 == f2 ->
	begin match can_be_built_list public l1 l2 with
	| None -> None
	| Some lst -> Some(FunApp(f1,lst))
	end
    | _ ->
        None

and can_be_built_list public l1 l2 =
  match l1, l2 with
  | [],[] -> Some []
  | hd1::tail1, hd2::tail2 ->
      begin
      match can_be_built public (hd1, hd2) with
        | None -> None
        | Some ca ->
            match can_be_built_list public tail1 tail2 with
              | None -> None
              | Some catail -> Some (ca::catail)
      end
  | _ -> Parsing_helper.internal_error "lists of different lengths in can_be_built_list"

let rec remove_first_in_public public = function
    [] -> []
  | (((c, a)::l) as l') ->
    if Terms.is_ground_public (fst a) && Terms.equal_terms (fst a) (snd a)
    then
      begin
        link c (TLink (fst a));
...
let update_term_list oldpub public tc_list =
  ...
  | ((c0, t0)::l0) ->
	...
	if public == oldpub then false else
	    ...
	    is_in_until l
    in
    if Terms.is_ground_public (fst t0) && Terms.equal_terms (fst t0) (snd t0)
    then
      begin
        Terms.link c0 (TLink (fst t0));
...
let rec do_red_nointeract f prev_state n =
  ...
  | Let(pat,t,p,q,occ) ->
      ...
      try
        auto_cleanup_red (fun () ->
          let t', name_params' = term_evaluation_name_params_and_match prev_state pat (OLet(occ)) t name_params in
          let p' = copy_process p in
          let name_params'' = update_name_params prev_state IfQueryNeedsIt name_params' pat in
          do_red_nointeract f { prev_state with
      ...
      if q == Nil then
        ...
        try
          auto_cleanup_red (fun () ->
            ...
            let name_params' = term_evaluation_name_params_true prev_state (OTest(occ)) t name_params in
            do_red_nointeract f
                    { prev_state with
                      subprocess = replace_at n (p, name_params', new_occs, facts, Nothing) prev_state.subprocess;
        ...
        try
          auto_cleanup_red (fun () ->
            let new_occs = (TestTag occ) :: occs in
            let (t', name_params') = term_evaluation_name_params prev_state (OTest(occ)) t name_params in
  ...
  | Event(FunApp(fs,l) as t,_,p,occ) ->
      ...
      let do_end prev_state new_occs new_facts t' =
        ...
        let (new_goal, event_in_goal, success) =
          (* We do not store the sid list here since they were only used for injective queries.
      ...
      begin
        ...
        try
		  ...
		  with Unify -> raise DerivBlocks
            ...
            | WithOcc -> Parsing_helper.internal_error "[Reduction_bipro.ml] Unexpected injective event."
          end
        with Unify ->
          f false { prev_state with
                    subprocess = remove_at n prev_state.subprocess;
...
let rec init_rule state tree =
  match tree.desc with
	...
	| Pred({ p_info = [Attacker _] | [AttackerBin _] | [Table _] | [TableBin _] }, _) ->
            (* We treat
...
let do_res_in cur_state seen_list rest_subprocess n current_cache_list name_params' new_occs facts tc pat p tc' mess_term public_status next_f    =
  ...
  try
    ...
    auto_cleanup_red (fun () ->
      ...
      let name_params'' = update_name_params cur_state Always name_params' pat in
      let p' = auto_cleanup (fun () -> copy_process p) in
...
let do_sync_res_io cur_state seen_list rest_subprocess n name_params' new_occs facts tc pat p tc' public_channel next_f   =
  ...
  let rec find_synchronous_output noutput = function
    ...
    | ((Output(tc2,t2,p2,out_occ),name_params2,occs2, facts2, cache_info2)::rest_subprocess2) ->
      begin
        (* when (p2 != Nil) || public_channel || not (is_equivalence_goal cur_state.goal) *)
        try
...
let rec find_in_out next_f cur_state n seen_list = function
  ...
  | ((Input(tc,pat,p,occ) as proc ,name_params,occs, facts, cache_info)::rest_subprocess) ->
	...
	| Nothing ->
	    ...
	    try
              auto_cleanup_red (fun () ->
		let (tc', name_params') = term_evaluation_name_params cur_state (OInChannel(occ)) tc name_params in
		let m =
		  if cur_state.current_phase < get_min_choice_phase() then
...
let update_hyp_not_matched old_phase new_phase cur_state =
  ...
  List.fold_left (fun state hyp ->
	...
	begin
	  let tl' = Evaluation_helper.decompose_tuple t in
	  List.fold_left (fun state t' ->
	    let bi_t' = (t',t') in
	    match t' with
	      FunApp({ f_cat = Name _; f_private = false },[]) ->
		{ state with public = (t',bi_t') :: state.public }
	    | _ ->
		if (not (is_in_public  state.public bi_t' = None)) then
		  state
		else
		  match can_be_built state.public bi_t' with
		  | None ->
		      ...
		      { state with
                        public = (recipe,bi_t') :: state.public;
                        hyp_not_matched = (Some recipe, Pred(p,[t']))::state.hyp_not_matched }
		  | Some recipe ->
		      { state with public = (recipe,bi_t') :: state.public }
			) state tl'
        end
    | (_, Pred({ p_info = [ AttackerBin(phase,_)] } as p, [t1;t2])) when phase > old_phase && phase <= new_phase ->
	begin
	  ...
	  let bi_tl' = decompose_tuple (t1, t2) in
	  List.fold_left (fun state ((t1', t2') as bi_t') ->
	    match bi_t' with
	      (FunApp({ f_cat = Name _; f_private = false },[]),
	       FunApp({ f_cat = Name _; f_private = false },[])) when
	      equal_terms_modulo t1' t2' ->
		{ state with public = (t1', bi_t') :: state.public }
	    | _ ->
		if (not (is_in_public state.public bi_t' = None)) then
		  state
		else
	...
	if List.exists (equal_bi_terms_modulo (t,t)) state.tables then
	  state
	else
	  { state with
            tables = (t,t) :: state.tables;
            hyp_not_matched = (None, fact)::state.hyp_not_matched }
    | (_, (Pred({ p_info = [ TableBin(phase) ] }, [t1;t2]) as fact)) when phase > old_phase && phase <= new_phase ->
	if List.exists (equal_bi_terms_modulo (t1,t2)) state.tables then
	  state
	else
	  { state with
            tables = (t1,t2) :: state.tables;
            hyp_not_matched = (None, fact)::state.hyp_not_matched }
    | _ ->
	{ state with
...
let build_goal tree = function
  | CorrespQEnc _ ->
      let (fact_list,id_goals) = get_corresp_goals tree in
      let goal_list =
...
let rec extract_conclusion_query restwork = function
  ...
  | QEvent (QNeq ((t1,_),(t2,_))) -> restwork ([], [], Terms.constraints_of_neq t1 t2, [], [])
  | QEvent (QGeq ((t1,_),(t2,_))) -> restwork ([], [], Terms.constraints_of_geq t1 t2, [], [])
  | QEvent (QIsNat t) -> restwork ([],[],Terms.constraints_of_is_nat t,[],[])
  | QEvent (QEq ((t1,_),(t2,_))) -> restwork ([], [], Terms.true_constraints, [t1], [t2])
  | QEvent((QSEvent2(t1,t2))) -> restwork ([t1,t2],[],Terms.true_constraints,[],[])
  | QEvent (QGr _) -> Parsing_helper.internal_error "[reduction_bipro.ml >> extract_conclusion_query] There should not be any strict disequalities in correspondance queries for biprocess."
...
let rec check_query_falsified_rec restwork event_table concl_q evl goall =
  ...
  | ev::rest_evl, (Fact(goal,_,_) | EventGoal(goal,_))::rest_goall ->
      let (l,l') =
        match ev, goal with
          QFact(p,_,l,_), Pred(p',l') when p == p' -> l,l'
    ...
    Reduction_helper.instantiate_natural_predicates (fun () ->
      ...
      let res =
	    ...
	    let state = update_hyp_not_matched (-1) 0 state in
            ...
            let verif_axioms = Lemma.check_axioms final_state axioms in
