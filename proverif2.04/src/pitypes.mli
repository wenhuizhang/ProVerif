...
(* In the queries generated from the input file, the instances of QSEvent(inj,ord_fun,occ,at,ev)
   satisfy the following properties:
    - ord_fun = []
    - occ = None
    - if at = Some t then t is a variable, i.e. t = Var v for some v.
   The instances of QFact(p,ord_fun,args,at) satisfy the following properties:
    - ord_fun = []
    - if at = Some t then t is a variable, i.e. t = Var v for some v.
   The instances of QGe(t1,t2) satisfy the following property:
    - t1 and t2 are both variables of type time.

   In the encoded queries (and so the queries that are verified in piauth.ml, reduction.ml, reduction_bipro.ml),
   the instances of QSEvent(inj,ord_fun,occ,at,ev) and QFact(p,ord_fun,args,at) satisfy the following properties:
    - at = None
    - if the instances occur in the premise of the query then ord_fun = [].
   Moreover, there is no instance of QGe(t1,t2).
*)

type term_e = term * Parsing_helper.extent

type event =
    QSEvent of
      int option (* None when not injective. Some k indicates injective with k as index. *) *
      ordering_function (* The ordering function *) *
      term option (* The occurrence *) *
      term_e option (* Temporal variable *) *
      term (* The event *)
  | QSEvent2 of term * term (* Event for biprocesses *)
  | QFact of
      predicate *
      ordering_function  (* The ordering function *) *
      term list (* The arguments *) *
      term_e option (* Temporal variable *)
  | QNeq of term_e * term_e
  | QEq of term_e * term_e
  | QGeq of term_e * term_e
  | QGr of term_e * term_e
  | QIsNat of term
...
type realquery_e = realquery * Parsing_helper.extent
...
type query_e = query * Parsing_helper.extent
...
and t_one_lemma =
  {
    ql_query : query_e; (* the encoded query *)
    ...
    ql_original_query : query_e option; (* The original query *)
...
and t_lemmas_state =
  {
    ...
    remove_events : Types.t_remove_events_for_lemma
  }
...
and t_query =
  ...
  | CorrespQuery of query_e list * t_solve_corresp_status
        (* Correspondence query. Several queries can be grouped
           together in the query list. *)
  | CorrespQEnc of (query_e(*original query*)  * query_e(*encoded query*)) list * t_solve_corresp_status
        (* Correspondence query, encoded as another correspondence query.
           Used for "public_vars" and for "query secret x [reachability]" *)
  | ChoiceQEnc of query_e (* original query, encoded as an equivalence
...
and t_construction =
  ...
  | Move of moved_ins * t_process_desc
...
and t_pi_state =
    { pi_process_query : t_process_query;
      ...
      pi_all_barrier_tags : (int * (string * Parsing_helper.extent)) list;
      ...
      pi_event_status_table : eventstatus HashtblSymbol.t opt_set;
      ...
      pi_get_nounif : t_pi_state -> (fact_format * nounif_value * nounif_op) list;
      ...
      pi_lemma: t_lemmas list;
        (* List of lemmas *)
      pi_original_axioms: (realquery_e * bool) list;
        (* List of original axioms and restrictions (before simplification but after encoding).
           When the boolean is true then it is an axiom, otherwise a restriction.
           Used for trace reconstruction. *)
...
type 'a goal_t =
  | CorrespGoal of corresp_goal_t list * int list
...
type group_summary_t =
    { sum_queries : (t_process_query * query_res) list;
      ...
      sum_axioms : (t_process_query * bool) list }
