...
let rec decompose_tuple = function
  | FunApp({f_cat = Tuple },l) ->
      List.concat (List.map decompose_tuple l)
  | t -> [t]

let rec can_be_built public t =
  (* When there is an equational theory, it may happen that
     this function does not manage to build [t], even though it
  ...
  with Not_found ->
    match t with
    | FunApp({f_private = false; _} as f,l) ->
	begin match can_be_built_list public l with
	| None -> None
	| Some lst -> Some(FunApp(f,lst))
	end
    | _ ->
...
and can_be_built_list public = function
  | [] -> Some []
  | hd::tail ->
      match can_be_built public hd with
        | None -> None
        | Some ca ->
            match can_be_built_list public tail with
              | None -> None
              | Some catail -> Some (ca::catail)
