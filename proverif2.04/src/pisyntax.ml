	...
	if fst r.f_type == Param.tmp_type then
	  ...
	  begin
		...
		let p = List.map (function
		  ...
		  | _ -> Terms.new_var_def_term Param.any_type) sl in
...
let check_event state ((f,e),n) =
	...
	match tl0 with
	  [PGFunApp(id, tl),_] ->
	    if !Param.key_compromise == 0 then
	      QSEvent(None,[], None,None,FunApp((find_event_arity state id (List.length tl)),
				      List.map (check_query_term state) tl))
	    else
	      QSEvent(None,[], None,None,FunApp((find_event_arity state id (1+List.length tl)),
				    (Terms.new_var_def_term Param.any_type)::
				    (List.map (check_query_term state) tl)))
  ...
  | PGSimpleFact(("evinj",e'),tl0) ->
	...
	match tl0 with
	  [PGFunApp(id, tl),_] ->
	    if !Param.key_compromise == 0 then
	      QSEvent(Some (Param.fresh_injective_index ()),[], None,None,FunApp((find_event_arity state id (List.length tl)),
				   List.map (check_query_term state) tl))
	    else
	      QSEvent(Some (Param.fresh_injective_index ()),[], None,None,FunApp((find_event_arity state id (1+List.length tl)),
				   (Terms.new_var_def_term Param.any_type)::
				   (List.map (check_query_term state) tl)))
  ...
  | PGSimpleFact(("attacker",_), tl) ->
	...
	input_error "arity of predicate attacker should be 1" e;
      ...
      QFact(att_n,[], List.map (check_query_term state) tl,None)
  | PGSimpleFact(("mess",_), tl) ->
	...
	input_error "arity of predicate mess should be 2" e;
      ...
      QFact(mess_n,[], List.map (check_query_term state) tl,None)
  | PGSimpleFact(p, tl) ->
      let p = get_pred state.q_pred_env p (List.length tl) in
      QFact(p,[], List.map (check_query_term state) tl,None)
...
let check_real_query_top state = function
    PBefore(ev, hypll) ->
      ...
      let ev'' =
	match ev' with
	  QNeq _ | QEq _ | QGeq _ | QGr _ | QIsNat _ -> user_error "Inequalities, disequalities or equalities cannot occur before ==> in queries"
	| QFact _ -> ev'
	| QSEvent _ when !Param.key_compromise == 0 -> ev'
	| QSEvent(inj,[], None,None,FunApp(f, sid::l)) ->
	    QSEvent(inj,[], None, None,FunApp(f, non_compromised_session::l))
	| QSEvent _
        | QSEvent2 _->
...
let transl_query events pred_env glob_table q pi_state =
  ...
  let q'' = List.filter (function Pitypes.PutBegin(_,[]),_ -> false | _ -> true) q' in
  CorrespQuery(List.map (fun (q, ext) -> (Reduction_helper.check_inj_coherent q, ext)) q'',Param.dummy_solve_status)
	   ...
	   if fst r.f_type == Param.tmp_type then
		 ...
		 Name { prev_inputs_meaning = sl } ->
		   let p = List.map (fun _ ->
		     FAny (Terms.new_var_def Param.any_type)) sl in
		   let r_arity = List.length (fst r.f_type) in
		   if List.length p != r_arity then
		     internal_error ("name " ^ s ^ " expects " ^ (string_of_int r_arity) ^ " arguments, but has " ^ (string_of_int (List.length p)) ^ " elements in prev_inputs_meaning");
		   FFunApp(r, p)
...
let all_barrier_tags = ref []

let corresp_query_list = ref ([] : Piptree.query_e list list)
let equiv_query_list = ref ([] : t_query list)
let need_vars_in_names = ref ([] : (funsymb * string * Parsing_helper.extent) list)
...
let nounif_list = ref ([] : (Piptree.gfact_format * nounif_value * (Piptree.ident * Piptree.gformat_e) list) list)
...
let check_fun_decl is_tuple (name,ext) arity is_private =
  ...
  in
  Database.record_fun r;
  Hashtbl.add funs name r
...
let check_may_fail_var env s =
  if not (Hashtbl.mem env s) then
    begin
      let r = Terms.new_var ~may_fail:true s Param.any_type in
      Hashtbl.add env s r
    end
...
let f_eq_tuple f s ext =
  match f.f_cat with
    Eq _ | Tuple -> ()
  | _ -> input_error ("function " ^ s ^ " has been defined by reduction. It should not appear in equations or clauses") ext
...
let check_red_may_fail tlist is_private =
  ...
  let may_fail_vars = new_n_list (fun () -> Terms.new_var_def_term ~may_fail:true Param.any_type) arity
...
let check_red tlist is_private =
  ...
  let red_list = List.map (function
    | ((PFunApp((f',ext'),l1),_), t2) ->
        ...
        (lhs, rhs,Terms.true_constraints)
...
let rec check_one_hyp (hyp_accu,constra_accu) env (fact, ext) =
  match fact with
  | PSNeq(t1,t2) -> (hyp_accu, { constra_accu with neq = [check_eq_term f_any  false true env t1, check_eq_term f_any  false true  env t2] ::
		     constra_accu.neq})
...
let free_name s ext =
   try
     List.find (fun r -> Terms.get_fsymb_origname r = s) (!freenames)
   with Not_found ->
...
let rec check_process seen_macros env = function
  ...
  | PRestr((s,ext),p) ->
      let r = Terms.create_name s (Param.tmp_type, Param.any_type) true in
      Database.record_name r;
      Hashtbl.add (!glob_table) s (EName r);
      if (StringMap.mem s env) && (!Param.tulafale != 1) then
  ...
  | PEvent((i,ext),l,p) ->
	...
	Event(FunApp(f, (Terms.new_var_def_term Param.any_type) :: (List.map (check_term env) l)), (None, env), check_process seen_macros env p, Terms.new_occurrence())
  | PPhase(n, p) ->
      let occ' = Terms.new_occurrence() in
  ...
  | PBarrier(n, tag, p) ->
      ...
      in
      all_barrier_tags := (n, tag') :: (!all_barrier_tags);
	...
	if (s' <> "") && (s'.[0] != '!') then
	  begin
		...
		EName r ->
		  ...
		  need_vars_in_names := (r, s',ext') :: (!need_vars_in_names)
	      |	_ -> ()
	    with Not_found ->
	      ()
	  end;
...
let set_need_vars_in_names () =
  List.iter (List.iter (fun (q, _) -> nvn_q q)) (!corresp_query_list);
...
let rec check_one = function
  ...
  | (NoUnif (fact,nv,bl)) ->
      nounif_list := (fact, nv, bl) :: (!nounif_list)
  ...
  | (Free (il,b)) ->
      List.iter (fun (i,ext) ->
	if (List.exists (fun r -> Terms.get_fsymb_origname r = i) (!freenames)) && (!Param.tulafale != 1) then
	  input_error ("free name " ^ i ^ " already declared") ext;
	ignore(add_free_name (not b) i)) il
  | (Clauses c) ->
      List.iter check_clause c
  | (Param((p,ext),v)) -> interpret_setting (p,ext) v
...
let parse_file s =
  ...
  in
  let max_phase =
    List.fold_left (List.fold_left (fun accu (q,_) ->
  ...
  let process_query =
	...
	SingleProcessSingleQuery({ proc = r; bi_pro = true; display_num = None; construction = Initial_process }, ChoiceQuery)
      end
    else
      let queries =
	(List.rev_map (fun q ->
	...
	(List.rev (!equiv_query_list))
      in
      SingleProcess({ proc = r; bi_pro = false; display_num = None; construction = Initial_process }, queries)
  in
  let pi_state =
    { pi_process_query = process_query;
      ...
      pi_all_barrier_tags = !all_barrier_tags;
