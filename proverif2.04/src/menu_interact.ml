...
let rec anal_file s0 =
  ...
  let build_state_from p =
	...
	begin
	  try
	    ignore (Terms.choice_in_proc 1 p);
	    let i_choice =
              Menu_helper.question_box
		~title:"Choice/diff present in your process"
		~buttons:["Left"; "Right"; "Biprocess"]
		("Would your like to:"
	  ...
	  with Terms.DiffPattern ->
	    (* The function [choice_in_proc] raises [DiffPattern] when 
	       there is diff/choice as pattern constructor:
	       such biprocesses cannot be split into 2 monoprocesses.
	       In this case, we always simulate the biprocess *)
	    Param.bipro_i_mode := true;
	    p
	end
      else
  ...
  try
	...
	ignore (Menu_helper.question_box ~title:"Warnings" ~buttons:["OK"] messages)
      end;
    begin
      ...
      | Equivalence(p1, p2) ->
         begin
           match  Menu_helper.question_box ~title:"Choose a process to simulate" ~buttons:["First"; "Second"] "Simulate first or second process ?" with
...
let create_new_nonce () =
  ...
  with
  | Not_found -> Menu_helper.error_box "Type not defined"
  | Menu_helper.WrongChoice -> ()
  ...
  try
    Terms.auto_cleanup (fun () ->
	  ...
	  let state' = Menu_helper.add_public_list_dec_all_mode state recipe_term_list in
          if state' == state then
            Menu_helper.error_box "Term (or its components after decomposing data constructors) already in the public elements of the current state"
          else
	    ...
	    Menu_helper.error_box "The evaluation of the recipe fails"
  ...
  if !Param.trace_win_open = true then
    begin
      ...
      match Sys.command ("dot -Tpng " ^ dot_file_name ^ " -o " ^ png_file_name) with
        0 ->
         begin
           let pixbuf = GdkPixbuf.from_file_at_size png_file_name ~width:!png_width ~height:!png_height in
           !img#set_pixbuf pixbuf;
         end
      | _ ->
          Menu_helper.error_box "The call to Graphviz failed. Please check that Graphviz is correctly installed.\n(The program \"dot\" must be in the PATH.)";
          Param.trace_win_open := false
    end
  ...
  let rec callback_save () =
    let ok_cb dialog  () =
      match dialog#filename with
        None ->
          Menu_helper.error_box "Please use .png, .pdf, .jpg, or .eps format to save the file.";
          dialog#destroy ();
          callback_save()
      | Some s ->
         begin
           ...
           if (StringPlus.ends_with s_up ".PNG") || (StringPlus.ends_with s_up ".PDF") || (StringPlus.ends_with s_up ".JPG") || (StringPlus.ends_with s_up ".EPS") then
             begin
	       let last_3 = String.lowercase_ascii (String.sub s (String.length s - 3) 3) in
               ...
               with
                 0 -> dialog#destroy ();
               | _ ->
		   Menu_helper.error_box ("The call to " ^ (!Param.interactive_dot_command) ^ " failed. Please check that it is correctly installed.\n(The program \"" ^ (!Param.interactive_dot_command) ^ "\" must be in the PATH.)");
                   dialog#destroy ()
             end
           else
             Menu_helper.error_box "Please use .png, .pdf, .jpg, or .eps format to save the file.";
